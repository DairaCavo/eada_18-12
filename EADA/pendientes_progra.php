    ...
    <?php if ($result && mysqli_num_rows($result) > 0): ?>
        <?php while ($ticket = mysqli_fetch_assoc($result)): ?>
            <div class="ticket_container">
                ...
                <a href="respuesta_programador.php?id_ticket=<?= htmlspecialchars($ticket['id_ticket']) ?>&id_usuario=<?= htmlspecialchars($ticket['id_usuario']) ?>">    <button class="boton_responder">Responder</button>
        </a>
            </div>
        <?php endwhile; ?>
    <?php else: ?>
